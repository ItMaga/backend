...
/*GET Вывод информации о контракте по идентификатору контракта*/
router.get('/get', function(req, res, next) {
    var id = req.get('Id');

    connection.query('SELECT id_contract, id_entry_contract, name, employee, dateStart, dateEnd, contract.price, status ' +
        'FROM contract, work, entry WHERE id_contract = ? AND contract.id_entry_contract = entry.id_entry AND contract.id_work_contract = work.id_work' +
        ' ORDER BY id_contract desc', id, function (error, results, fields) {
        if(error){
            console.log('error 1');
            res.send(JSON.stringify({"status": 500, "error": error, "response": null}));
            //If there is error, we send the error in the error section with 500 status
...
/* PUT Изменение данных контракта */
router.put('/change', function(req, res, next) {
    ...
    connection.query('SELECT id_work FROM work WHERE name =  ?', work, function(error, results, fields) {
        if (error) {
            ...
            connection.query('UPDATE autoservice.contract SET id_entry_contract = ?, id_work_contract = ?, employee = ?,' +
                ' dateStart = ?, dateEnd = ?, price = ?, status = ? WHERE id_contract = ?',
                [id_entry_contract, id_work_contract, employee, dateStart, dateEnd, price, status, id], function(error, results, fields) {
                    if (error) {
                        console.log('error');
                        return res.send(JSON.stringify({"status": 500, "error": error, "response": null}));
                    } else {
